#Code by Uttam Keshri , and converted from .pynb to .py using https://jupyter.org/try
...
def renderConfusionMetrics ( ___model, _testData, _testLabels, enableTraining, ___train_gen, ___test_gen, __batch_size, __epochs, hdf5_testSaveFileName ):
    ...
    recall = tp/(tp+fn)*100
    specificity = tn/(tn+fp)*100 #Uttam_note: added specificity calculation 
    print('Accuracy: {}%'.format(acc))
    print('Precision: {}%'.format(precision))
    print('Recall/Sensitivity: {}%'.format(recall)) #Uttam_note: added sensitivity label
    print('Specificity {}%'.format(specificity)) #Uttam_note: added specificity calculation 
...
# disabling warnings
import logging
logging.getLogger('tensorflow').disabled = True #Uttam_note: Disable red warning lines seen at model architecture creation.
...
model_covid19PneumoniaDetector.load_weights('covid19_neural_network_weights_uttam.hdf5')
...
#Uttam_note establish custom_path for covid 19 test data
...
# Reporting on accuracies
renderConfusionMetrics ( model_covid19PneumoniaDetector, test_data_d, test_labels_d, False, train_gen_d, test_gen_d, batch_size, 11, 'covid19_neural_network_weights_uttam_v2.hdf5' )
